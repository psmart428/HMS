...
export default function VisitorDetails() {
  const { onCloseModuleMenu, modalType, openId, countries } =
    useModuleContext();
  const { isLoading, visitorDetails } = useGetVisistor(openId, modalType);
        ...
        <div className="p-6 overflow-y-auto max-h-[80vh]">
          <div className="text-center mb-6">
            ...
            <h4 className="text-2xl font-bold text-gray-800 dark:text-white">
              {visitorDetails?.fullName || "Visitor Name"}
            </h4>
          ...
          <VisitorCard visitorDetails={visitorDetails} countries={countries} />
        </div>
      </div>
    </div>
  );
